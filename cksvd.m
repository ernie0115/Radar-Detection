function [D,Gamma,err,gerr] = cksvd(params,varargin)

%  CKSVD K-SVD dictionary training for complex numbers. 

%  [D,GAMMA] = cksvd(PARAMS) runs the K-SVD dictionary training algorithm on
%  the specified set of complex signals, returning the trained dictionary D and the
%  signal representation matrix GAMMA.

%  CKSVD has two modes of operation: sparsity-based and error-based. 

%  For sparsity-based minimization, the optimization problem is given by
%
%      min  |X-D*GAMMA|_F^2      s.t.  |Gamma_i|_0 <= T
%    D,Gamma
%
%  where X is the set of training signals, Gamma_i is the i-th column of
%  Gamma, and T is the target sparsity. 

%  For error-based minimization, the optimization problem is given by
%
...
%  [D,GAMMA,ERR] = cksvd(PARAMS) also returns the target function values
...
%
%  [D,GAMMA,ERR,GERR] = cksvd(PARAMS) computes the target function values on
...
%
%  [...] = cksvd(...,VERBOSE) where VERBOSE is a character string, specifies
...
%  example, cksvd(PARAMS,'tr') prints the iteration number, number of
...
%
%  [...] = cksvd(...,MSGDELTA) specifies additional messages to be printed
...
%  [3] J. S. Turek, M. Elad, and I. Yavneh, "Clutter Mitigation in 
%      Echocardiography using Sparse Signal Separation", International 
%      Journal on Biomedical Imaging, April 2015.
%
%  See also KSVDDENOISE, OMPDENOISE, COMP, COMP2.
...
global parallelmode PARALLEL_ON PARALLEL_OFF
...
% run comp in parallel %
if (isfield(params,'parallel')  && params.parallel~=0)
%   ompparams{end+1} = 'parallel';
%   ompparams{end+1} = params.parallel;
  parallelmode = PARALLEL_ON;
else
  parallelmode = PARALLEL_OFF;
end
...
if (isfield(params,'initdict'))
  ...
  data_ids = find(colnorms_squared(data) > 1e-48);   % ensure no zero data elements are chosen
...
for iter = 1:iternum
  ...
  if (printreplaced)
    info = sprintf('%s, replaced %d+%d atoms', info, sum(replaced_atoms),cleared_atoms);
  end
...
[gamma_j, data_indices] = csprow(Gamma, j);

if (length(data_indices) < 1)
  maxsignals = 5000;
  perm = randperm(length(unused_sigs));
  perm = perm(1:min(maxsignals,end));
  E = sum(abs(X(:,unused_sigs(perm)) - D*Gamma(:,unused_sigs(perm))).^2);
  [d,i] = max(E);
...
if (exactsvd)
  ...
  gamma_j = s*conj(gamma_j); % conj because it is U*S*V' (' = conj with transpose)  
else 
%   atom = ccollincomb(X,data_indices,gamma_j') - D*(smallGamma*gamma_j') + Dj*(gamma_j*gamma_j');
%   atom = atom/norm(atom);
%   gamma_j = conj(crowlincomb(atom,X,1:size(X,1),data_indices)) - conj((atom'*D)*smallGamma) + conj((atom'*Dj)*gamma_j);

    E = X(:,data_indices) - D*smallGamma + Dj*gamma_j;
    atom = E*gamma_j';
    atom = atom/norm(atom);
    gamma_j = atom'*E;
end
...
global parallelmode PARALLEL_ON
global MEM_HIGH memusage
global ompfunc ompparams

if (parallelmode == PARALLEL_ON)
%     localdata = distributed(data);
%     localparams = ompparams;
%     localompfunc = ompfunc;
%     if (memusage < MEM_HIGH)
%         spmd
%             localGamma = localompfunc(D,localdata,G,thresh,localparams{:});
%         end
%     elseif (codemode == CODE_SPARSITY)
%         spmd
%             localGamma = localompfunc(D'*localdata(labindex),G,thresh,localparams{:});
%         end
%     else
%         error('should not reach this');
%     end
%     Gamma = gather(localGamma);
    numdata = size(data,2);
    datachunk = ceil(numdata/4);
    localGamma = cell(1,4);
    localdiv = ones(4,1)*datachunk;
    localdiv(end) = localdiv(end) - (sum(localdiv)-numdata);
    localdata = mat2cell(data,size(data,1),localdiv);
    localompfunc = ompfunc;
    localparams = ompparams;
    if (memusage < MEM_HIGH)
        parfor i = 1:4
            localGamma{i} = localompfunc(D,localdata{i},G,thresh,localparams{:});
        end
    else
        if (codemode == CODE_SPARSITY)
            parfor i = 1:4
            ...
            localXtX = mat2cell(XtX,size(XtX,1),localdiv);
            parfor i = 1:4
                localGamma{i} = localompfunc(D'*localdata{i},localXtX,G,thresh,localparams{:});
            end
        end 
    end
    Gamma = cell2mat(localGamma);
else
    if (memusage < MEM_HIGH)
...
for i = 1:length(blocks)-1
  err(blocks(i):blocks(i+1)-1) = sum(abs((X(:,blocks(i):blocks(i+1)-1)-D*Gamma(:,blocks(i):blocks(i+1)-1))).^2);
end

cleared_atoms = 0;
usecount = sum(abs(Gamma)>1e-48, 2);
...
%   if ( (max(Gj.^2)>muthresh^2 || usecount(j)<use_thresh) && ~replaced_atoms(j) )
  if ( (max(abs(Gj).^2)>muthresh^2 || usecount(j)<use_thresh) && ~replaced_atoms(j) )
    [y,i] = max(err(unused_sigs));
%     D(:,j) = X(:,unused_sigs(i)) / norm(X(:,unused_sigs(i)));
    D(:,j) = X(:,unused_sigs(i)) / sqrt(sum(conj(X(:,unused_sigs(i))).*X(:,unused_sigs(i))));
...
for i = 1:blocksize:size(X,2)
  ...
  err2(blockids) = sum(abs(X(:,blockids) - D*Gamma(:,blockids)).^2);
end
...
for i = 1:blocksize:size(X,2)
  blockids = i : min(i+blocksize-1,size(X,2));
  Y(blockids) = sum(abs(X(:,blockids)).^2);
end
